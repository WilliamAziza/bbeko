...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled 
        ? 'bg-gradient-to-r from-slate-50 to-blue-50/80 backdrop-blur-md border-b border-gray-200/50 shadow-lg' 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          <button
            className={`md:hidden transition-colors p-3 rounded-xl ${
              isScrolled 
                ? 'text-gray-900 hover:bg-blue-100 focus:ring-blue-500' 
        ...
        {isMenuOpen && (
          <div className={`md:hidden pb-4 border-t transition-colors ${
            isScrolled ? 'border-gray-200/50 bg-slate-50/80' : 'border-white/20 bg-black/20'
          }`}>
              ...
              {navLinks.map((link, index) => (
                <a
                  ...
                  className={`block px-3 py-2 text-lg font-medium rounded-lg transition-colors ${
                    isScrolled 
                      ? 'text-gray-800 hover:text-blue-600 hover:bg-blue-100 font-semibold' 
